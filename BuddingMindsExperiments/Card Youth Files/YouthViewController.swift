...
class YouthViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    var model = YouthCardModel()
    var csvWriter = YouthCSVWriter()
    var pHandler = YouthPausedHandler()
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        let path = Bundle.main.path(forResource: "Youthcorrect-sound", ofType: "wav")!
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        // Returns cell
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "FlippableCardCell", for: indexPath) as! YouthCardCollectionViewCell
    ...
    func sendToCsv(cell: YouthCardCollectionViewCell, card: YouthCard) {
        // 1 - experiment name
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let cell = collectionView.cellForItem(at: indexPath) as! YouthCardCollectionViewCell
        let card = cardArray[indexPath.row]
    ...
    func getAverageTouches() -> Float {
        ...
        if cardArray.count == (numGreyedOutYouth + numFlippable) {
            div = Float(numFlippableYouth)
        } else {
            div = Float(numGreyedOutYouth)
        }
    ...
    func pause() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(1)) {
            self.performSegue(withIdentifier: "gameToPauseYouth", sender: self)
        }
    }
    ...
    func checkForMatch(_ flippedIndex:IndexPath) {
        ...
        let cell = collectionView.cellForItem(at: firstCard!) as? YouthCardCollectionViewCell
                    ...
                    if self.phase == 5 {
                        DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(2)) {
                            ...
                            if skippingPhases {
                                self.writeBlockTime()
                                DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(2)) {
                                    self.performSegue(withIdentifier: "gameToEndYouth", sender: self)
                                }
                            } else {
                                self.phase += 1
                                self.writeBlockTime()
