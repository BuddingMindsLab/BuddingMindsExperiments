...
var participantIdYouth = ""
var experimentNameYouth = "version1"
var unpausingYouth = false

extension UIViewController {
    
    // Original toast function made by Stack Overflow user "Mr.Bean",
    // edited for my purposes
    func showToastYouth(message : String) {
...
class YouthIDScreenViewController: UIViewController, UITextFieldDelegate {
    ...
    func checkPaused() {
        if pausedHandler.checkPaused(id: participantIdYouth) {
            performSegue(withIdentifier: "idToUnpauseCheckYouth", sender: self)
        } else {
            instructionToShowYouth = 1
            
            // Leave this code in to show the instruction page before starting the experiment
            performSegue(withIdentifier: "idToInstrYouth", sender: self)
