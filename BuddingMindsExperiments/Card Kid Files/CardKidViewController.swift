...
class CardKidViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    var model = KidCardModel()
    var csvWriter = KidCSVWriter()
    var pHandler = KidPausedHandler()
    
    var cardArray = [KidCard]()
    var targetArray = [KidCard]()
    ...
    override func viewDidLoad() {
        ...
        if skippingPhasesKid {
            print(Int(participantIdKid)! % 32)
            // These participants will do the second half
            if Int(participantIdKid)! % 32 > 15 {
                phase = 2
                
                if unpausingKid {
                    phase = 4
                    pHandler.removePaused(id: "\(participantIdKid)-\(csvWriter.getLatestVersion())")
                }
            } else {
                phase = 1
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        participId = Int(participantIdKid)!
        expName = experimentNameKid
        
        collectionView.delegate = self
        collectionView.dataSource = self
        
        // Get the correct-sound.wav file and load it into player
        let path = Bundle.main.path(forResource: "correct-sound_Kid", ofType: "wav")!
        ...
        } else if (phase == 3 || phase == 4) && unpausingKid {
            let file = model.getVidName(phase: phase)
            playVideo(from: file)
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        // Returns cell
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "FlippableCardCell", for: indexPath) as! KidCardCollectionViewCell
    ...
    func sendToCsv(cell: KidCardCollectionViewCell, card: KidCard) {
        // 1 - experiment name
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let cell = collectionView.cellForItem(at: indexPath) as! KidCardCollectionViewCell
        let card = cardArray[indexPath.row]
    ...
    func pause() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(1)) {
            self.performSegue(withIdentifier: "gameToPauseKid", sender: self)
        }
    }
    ...
    func checkForMatch(_ flippedIndex:IndexPath) {
        ...
        let cell = collectionView.cellForItem(at: firstCard!) as? KidCardCollectionViewCell
                    ...
                    if self.phase == 5 {
                        DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(2)) {
                            ...
                            if skippingPhases {
                                self.writeBlockTime()
                                DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(2)) {
                                    self.performSegue(withIdentifier: "gameToEndKid", sender: self)
                                }
                            } else {
                                self.phase += 1
                                self.writeBlockTime()
                        ...
                        if self.attempts < 3 || self.curAverageTouches != 1 || self.prevAverageTouches != 1 {
                            ...
                            if (self.phase == 3 && !unpausingKid) || (skippingPhases && self.phase == 4 && !unpausingKid) {
                                self.pause()
                            } else {
                                self.targetArray = self.model.getTargetCards(phase: self.phase)
                                // Set target card image
