...
extension OutputStream {
    func writeKid(_ string: String, encoding: String.Encoding = .utf8, allowLossyConversion: Bool = false) -> Int {
...
//        #endif
    ...
    func writeLine(elements: [String]) {
        if !writtenHeader && !unpausingKid {
            ...
            writtenHeader = true
            writeHeader()
        } else if !writtenHeader && unpausingKid {
    ...
    func writeHelper(str: String, app: Bool) {
        let name = "\(experimentName)-\(participantIdKid)-\(version).csv"
    ...
    func getLatestVersion() -> Int {
        ...
        do {
            ...
            for item in items {
                if item.hasPrefix("\(experimentName)-\(participantIdKid)") {
                    if item.contains("-") {
