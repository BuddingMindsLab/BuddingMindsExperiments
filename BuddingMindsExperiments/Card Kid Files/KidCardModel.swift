...
func getRSKid() -> GKMersenneTwisterRandomSource {
    return GKMersenneTwisterRandomSource(seed: UInt64(participantIdKid)!)
}

// This variable represents if we want to skip phases and only test each participant on half
var skippingPhasesKid = false
...
class KidCardModel{
    ...
    var pink = "pattern - pink_Kid.jpg"
    var blue = "pattern - blue_Kid.png"
    var orange = "pattern - orange_Kid.png"
    ...
    func getMatchingCards(phase: Int) -> [KidCard] {
        
        if doneOnce == false && !unpausingKid {
        ...
        } else if ((phase == 3 && unpausingKid) || (phase == 4 && unpausingKid)) && !doneOnce {
            doneOnce = true
    ...
    func getCardSet(phase: Int) -> [KidCard] {
        if phase == 1 || phase == 5 {
            return cardSets[0]
    ...
    func getTargetCards(phase: Int) -> [KidCard] {
    ...
    func shuffleTargetCards(old: [KidCard]) -> [KidCard] {
    ...
    func getVidName(phase: Int) -> String {
        ...
        if phase == 1 || ((phase == 3 && unpausingKid) || (phase == 4 && skippingPhases && unpausingKid)) {
            first = "Start"
        } else {
            if skippingPhases {
                if phase == 1 {
