...
var participantIdKid = ""
var experimentNameKid = "version1"
var unpausingKid = false

extension UIViewController {
    
    // Original toast function made by Stack Overflow user "Mr.Bean",
    // edited for my purposes
    func showToastKid(message : String) {
...
class KidIDScreenViewController: UIViewController, UITextFieldDelegate {
    ...
    func checkPaused() {
        if pausedHandler.checkPaused(id: participantIdKid) {
            performSegue(withIdentifier: "idToUnpauseCheckKid", sender: self)
        } else {
            instructionToShow = 1
            
            // Leave this code in to show the instruction page before starting the experiment
            performSegue(withIdentifier: "idToInstrKid", sender: self)
